...
public class YSZoomView: UIControl, UIScrollViewDelegate {
    ...
    private let animDuration: NSTimeInterval = 0.2 // 动画时长
    
    // MARK: - Public property
    
    // 图片开始时的frame
    public var originFrame: CGRect = CGRectZero
    // 要显示的图片
    public var image: UIImage? {
        didSet {
            if let _image = image {
                if self.originFrame == CGRectZero {
...
//                UIApplication.sharedApplication().keyWindow?.windowLevel = UIWindowLevelNormal
    ...
    @objc func doubleTapClick(tap: UITapGestureRecognizer) {
        self.touchX = tap.locationInView(tap.view).x
        self.touchY = tap.locationInView(tap.view).y
